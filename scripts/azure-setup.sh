...
echo ""
echo "── 게시판(Forum) 추가 변수 (포털 또는 az containerapp update --set-secrets 로 수동 설정) ──"
echo "  Azure API:"
echo "    DATABASE_URL=postgresql+asyncpg://...   # Flexible Server + ssl"
echo "    JWT_SECRET=   # Vercel AUTH_SECRET 과 동일"
echo "    AZURE_STORAGE_CONNECTION_STRING=   # 이미지 업로드"
echo "    AZURE_STORAGE_CONTAINER=forum-uploads"
echo "    AZURE_STORAGE_PUBLIC_BASE_URL=   # 선택"
echo "  Vercel:"
echo "    AUTH_SECRET (= JWT_SECRET), AUTH_URL, AUTH_TRUST_HOST=true"
echo "    AUTH_GOOGLE_ID/SECRET, AUTH_DISCORD_ID/SECRET"
echo "  DB 스키마: cd backend && alembic -c alembic.ini upgrade head"
echo ""
