...
def override_service(monkeypatch):
    ...
    async def fake_get_opening_tiers(self, rating, speed, color):
        return [{"eco": "A00", "name": "Fake", "tier": "S", "white_wins": 1, "draws": 0, "black_wins": 0, "total_games": 1, "win_rate": 1.0, "draw_rate": 0.0, "tier_score": 1.0}]
...
def test_global_endpoint_success():
    ...
    resp = client.get("/api/v1/opening-tier/global", params={"rating": 1600, "speed": "blitz", "color": "white"})
