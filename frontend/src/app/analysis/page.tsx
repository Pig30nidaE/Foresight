...
function AnalysisContent() {
  ...
  return (
    <div className="space-y-8 max-w-4xl mx-auto">
      <div>
        <h1 className="text-2xl font-bold text-white mb-1">📊 내 게임 분석</h1>
      ...
      {isLoading && (
        <p className="text-zinc-400 animate-pulse text-center py-10">
          최근 게임을 불러오는 중...
        </p>
      )}
        ...
        <div className="space-y-6">
          ...
          {perf.top_openings.length > 0 && (
            <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6 space-y-4">
              <h2 className="text-lg font-semibold">오프닝 승률 분석</h2>
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden">
            <div className="px-4 py-3 border-b border-zinc-800">
              <h2 className="font-semibold">오프닝 상세</h2>
            </div>
            <table className="w-full text-sm">
              ...
              <tbody>
                {perf.top_openings.map((op) => (
                  <tr key={op.eco} className="border-b border-zinc-800/50 hover:bg-zinc-800/30 transition-colors">
                    <td className="px-4 py-3">
                      <span className="text-zinc-400 mr-2 font-mono">{op.eco}</span>
